...
export function FloatingPanelTrigger({
  ...
  return (
    <motion.button
      ...
      className={cn(
        "flex h-9 items-center border border-border bg-background px-3 text-foreground rounded-lg",
        className
      )}
      onClick={handleClick}
      ...
      {isOpen && (
        <>
          ...
          <motion.div
            ...
            className={cn(
              "fixed z-50 overflow-hidden border border-border bg-background shadow-lg rounded-xl",
              className
            )}
            style={{
...
function FloatingPanelTitle({ children }: { children: React.ReactNode }) {
...
export function FloatingPanelLabel({
  ...
  return (
    <motion.label
      ...
      className={cn(
        "block mb-2 text-sm font-medium text-foreground",
        className
      )}
    >
      {children}
...
export function FloatingPanelTextarea({
  ...
  return (
    <textarea
      id={id}
      className={cn(
        "h-full w-full resize-none rounded-md bg-transparent px-4 py-3 text-sm outline-none border border-input",
        className
      )}
      autoFocus
      value={note}
...
export function FloatingPanelSubmitButton({ className }: { className?: string }) {
  return (
    <motion.button
      className={cn(
        "relative ml-1 flex h-8 shrink-0 items-center justify-center rounded-lg border border-border bg-transparent px-2 text-sm text-muted-foreground transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:ring-2 active:scale-[0.98]",
        className
      )}
      type="submit"
...
export function FloatingPanelButton({
  ...
  return (
    <motion.button
      className={cn(
        "flex w-full items-center gap-2 rounded-md px-4 py-2 text-left text-sm hover:bg-accent",
        className
      )}
      onClick={onClick}
